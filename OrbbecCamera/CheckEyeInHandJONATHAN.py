#!/usr/bin/env python3
"""
Check Eye-in-Hand Calibration

This script verifies the eye-in-hand calibration by:
1. Capturing an image with the Orbbec camera
2. Detecting ArUco marker or ChArUco board and estimating its pose using solvePnP
3. Getting the current robot pose
4. Computing where the marker/board should be in the base frame using the calibration
5. Visualizing the results

Usage:
    # ChArUco mode
    python CheckEyeInHand.py --calibration ../output/eyeinhand_from_poses.npz --mode charuco

    # ArUco mode
    python CheckEyeInHand.py --calibration ../output/eyeinhand_from_poses.npz --mode aruco --aruco-size 0.05
"""
...
def estimate_aruco_pose_solvepnp(image: np.ndarray, K: np.ndarray, D: np.ndarray,
    ...
    Returns:
        Dictionary with: rvec, tvec, R_matrix, marker_id, reprojection_error
    """
    ...
    print("ArUco rvec:", rvec.T)
    print("ArUco tvec:", tvec.T)

    # Convert to rotation matrix
    R_matrix, _ = cv2.Rodrigues(rvec)
    print("ArUco R_matrix:\n", R_matrix)

    # Calculate reprojection error
    proj_points, _ = cv2.projectPoints(obj_points, rvec, tvec, K, D)
    proj_points = proj_points.reshape(-1, 2)
    reproj_error = np.linalg.norm(marker_corners - proj_points, axis=1).mean()
...
def estimate_charuco_pose_solvepnp(image: np.ndarray, K: np.ndarray, D: np.ndarray,
    ...
    print("ChArUco rvec:", rvec.T)
    print("ChArUco tvec:", tvec.T)

    # Convert to rotation matrix
    R_matrix, _ = cv2.Rodrigues(rvec)
    print("ChArUco R_matrix:\n", R_matrix)
...
def get_robot_pose(xarm: XArmAPI) -> Optional[Tuple[np.ndarray, np.ndarray]]:
    ...
    try:
        ...
        if code != 0:
            print(f"⚠️  xArm get_position failed, code={code}")
            return None
        print("xarm pos = ", pos)
...
def visualize_result(image: np.ndarray, pose_result: Dict, K: np.ndarray, D: np.ndarray,
                    marker_in_base: Optional[np.ndarray] = None, axis_len: float = 0.0500) -> np.ndarray:
    ...
    cv2.putText(vis, f"Reproj error: {pose_result['reprojection_error']:.2f}px", (10, y_offset),
               cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
    y_offset += 35
...
def main():
    ...
    print("="*70)
    print("Check Eye-in-Hand Calibration")
    print("="*70)
    print()
    ...
    if args.mode == 'charuco':
        ...
        print(f"✓ Mode: ChArUco Board {args.squares_x}x{args.squares_y}")
        print(f"  Square: {args.square_len*1000:.1f}mm, Marker: {args.marker_len*1000:.1f}mm")
    else:
        print(f"✓ Mode: Single ArUco Marker")
    ...
    print("="*70)
    print("Live View - Press 's' to capture and analyze, 'q' to quit")
    print("="*70)
    ...
    try:
        ...
        while True:
            ...
            # Show preview
            preview = frame.copy()
            # Draw a '+' at the image center
            try:
                h, w = preview.shape[:2]
                cx, cy = int(w * 0.5), int(h * 0.5)
                cv2.drawMarker(preview, (cx, cy), (0, 0, 255), markerType=cv2.MARKER_CROSS, markerSize=24, thickness=2)
            except Exception:
                pass
            # Debug stats overlay
            try:
                ch_means = np.mean(preview, axis=(0, 1))
                ch_stds = np.std(preview, axis=(0, 1))
                ch_mins = np.min(preview, axis=(0, 1))
                ch_maxs = np.max(preview, axis=(0, 1))
                dbg1 = f"BGR mean: [{ch_means[0]:.1f}, {ch_means[1]:.1f}, {ch_means[2]:.1f}]"
                dbg2 = f"std: [{ch_stds[0]:.1f}, {ch_stds[1]:.1f}, {ch_stds[2]:.1f}]"
                dbg3 = f"min: [{ch_mins[0]:.0f}, {ch_mins[1]:.0f}, {ch_mins[2]:.0f}] max: [{ch_maxs[0]:.0f}, {ch_maxs[1]:.0f}, {ch_maxs[2]:.0f}]"
                y0 = 30
                cv2.putText(preview, dbg1, (10, y0), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                y0 += 22
                cv2.putText(preview, dbg2, (10, y0), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                y0 += 22
                cv2.putText(preview, dbg3, (10, y0), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
            except Exception:
                pass
            cv2.putText(preview, "Press 's' to capture, 'q' to quit", (10, 30),
            ...
            if key == ord('q'):
                ...
                try:
                    ...
                    out_path = "../output/debug_orbbec_frame.png"
                ...
                try:
                    ...
                    print(f"Current frame size: {w}x{h}, channels: {c}")
                except Exception:
                    pass
                ...
                print(f"  Reprojection error: {pose_result['reprojection_error']:.2f} pixels")
                ...
                # Compute marker/board pose in camera frame (from solvePnP)
                R_marker2cam = pose_result['R_matrix']
                t_marker2cam = pose_result['tvec']
                T_marker2cam = to_homogeneous(R_marker2cam, t_marker2cam)

                label = "Marker" if args.mode == 'aruco' else "Board"
                print(f"\n📊 {label} pose in camera frame (from solvePnP):")
                print(f"  Translation: {t_marker2cam}")
                print(f"  Rotation:\n{R_marker2cam}")

                # Calculate ALL ChArUco corners on the board
                if args.mode == 'charuco':
                    # 5x7 board has (5-1)x(7-1) = 4x6 = 24 internal corners
                    # Generate all corner positions in board frame
                    all_corners_board_frame = []
                    for row in range(args.squares_y - 1):  # 6 rows of corners
                        for col in range(args.squares_x - 1):  # 4 columns of corners
                            x = col * args.square_len
                            y = row * args.square_len
                            all_corners_board_frame.append([x, y, 0])

                    all_corners_board_frame = np.array(all_corners_board_frame, dtype=np.float64)

                    print(f"\n📐 ChArUco board: {args.squares_x}x{args.squares_y} squares = {len(all_corners_board_frame)} corners")
                    print(f"  Square size: {args.square_len*1000:.2f}mm")

                # Evaluate all extrinsics
                best_name = None
                best_t = None
                results_lines = []
                for name, T_cam2gripper in extrinsics_list:
                    T_marker2base = T_gripper2base @ T_cam2gripper @ T_marker2cam
                    R_m2b, t_m2b = from_homogeneous(T_marker2base)
                    results_lines.append((name, t_m2b, R_m2b))
                    if best_t is None:
                        best_name, best_t = name, t_m2b

                print(f"\n📈 {label} pose in base per extrinsics:")
                for name, t_m2b, _ in results_lines:
                    print(f"  {name}: t = [{t_m2b[0]:.4f}, {t_m2b[1]:.4f}, {t_m2b[2]:.4f}] m")

                # Transform SELECTED board corners to base frame
                if args.mode == 'charuco':
                    print("\n" + "="*70)
                    print("SELECTED CHARUCO CORNERS IN BASE FRAME (mm):")
                    print("="*70)
                    T_marker2base = T_gripper2base @ extrinsics_list[0][1] @ T_marker2cam

                    # 1-based corner numbers to print (mapped from your list)
                    selected_corners = [
                        1, 3, 5, 7, 9, 11, 13,
                    ...
                    for out_idx, corner_num in enumerate(selected_corners, start=1):
                        idx = corner_num - 1
                        if idx < 0 or idx >= len(all_corners_board_frame):
                            continue
                        corner_board = all_corners_board_frame[idx]
                        corner_hom = np.append(corner_board, 1.0)
                        corner_base_hom = T_marker2base @ corner_hom
                        corner_base = corner_base_hom[:3] * 1000.0
                        print(f"corner {out_idx}: ({corner_base[0]:.2f}, {corner_base[1]:.2f}, {corner_base[2]:.2f})")
                    print("="*70)

                # Visualize using selected (first) extrinsics
                use_name, use_t = results_lines[0][0], results_lines[0][1]
                axis_len = args.aruco_size if args.mode == 'aruco' else float(board.getSquareLength()) * 2.0
                vis = visualize_result(frame, pose_result, K, D, use_t, axis_len)
                cv2.putText(vis, "Press 'c' to continue or 'q' to quit", (10, vis.shape[0] - 20),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)
                cv2.imshow("Check Eye-in-Hand", vis)

                # Wait for 'c' to continue or 'q' to quit
                while True:
                    key = cv2.waitKey(1) & 0xFF
                    if key == ord('c'):
                        break
