...
square_length = 0.041     # Square size (meters)
marker_length = 0.0326      # Marker size inside each square (meters)
image_dir = "ArUcoBoardcalib_images_left"
...
for fname in images:
    ...
    if ids is not None and len(ids) > 0:
        ...
        if retval and charuco_ids is not None and len(charuco_ids) >= 4:
            ...
            output_path = os.path.join(save_dir, f"charuco_ok_{os.path.basename(fname)}")
            cv2.imwrite(output_path, img_copy)
        else:
            print(f"⚠️ Not enough Charuco corners in {fname} (retval={retval})")
            # Save image showing only markers
            output_path = os.path.join(save_dir, f"charuco_fail_{os.path.basename(fname)}")
            cv2.imwrite(output_path, img_copy)
    else:
        print(f"⚠️ No markers detected in {fname}")
        output_path = os.path.join(save_dir, f"no_markers_{os.path.basename(fname)}")
        cv2.imwrite(output_path, img_copy)
...
if len(all_corners) < 5:
    ...
    print(f"\n Calibration data saved to '{save_dir}/charuco_calibration.npz'")
